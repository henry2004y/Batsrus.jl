...
im3 = axs[3].imshow(
    Z_scaled, origin = "lower", extent = [xi[1], xi[end], yi[1], yi[end]],
    norm = PyPlot.matplotlib.colors.LogNorm(vmin = vmin_g, vmax = vmax_g), aspect = "auto"
)
...
axs[4].scatter(
    v_para[mask_core][1:stride:end], v_perp_mag[mask_core][1:stride:end],
