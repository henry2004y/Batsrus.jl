...
function dist_select(
        fnameParticle, xC = -1.9, yC = 0.0, zC = -0.1, xL = 0.005, yL = 0.2, zL = 0.07;
        ParticleType = 'e', dir = "."
    )
    if ParticleType == 'i'
...
"""
function dist_scan(
        region, particle, ParticleType = 'i', plottype = 1; dir = ".",
    ...
    for iB in 1:nBox
        ...
        elseif plottype == 4
            h = hist2D(
                uPerpO, uPerpI, bins = nbin,
                norm = matplotlib.colors.LogNorm(), density = true, range = binRange
            )
        elseif plottype == 5
            h = hist2D(
                uPerpI, uPar, bins = nbin,
                norm = matplotlib.colors.LogNorm(), density = true, range = binRange
            )
        elseif plottype == 6
            h = hist2D(
                uPerpO, uPar, bins = nbin,
                norm = matplotlib.colors.LogNorm(), density = true, range = binRange
            )
        else
        ...
        title(
            @sprintf(
                "%d, x[%3.3f,%3.3f], z[%3.3f,%3.3f]", iB, region[1, iB],
...
function plotExCut(
        filefield::String, region, xC, yC, zC, xL, yL, zL;
...
function dist_plot(pType = 'e')
    ...
    # Electron
    if pType == 'e'
        region[:, 1] = [-1.93, -1.925, -0.08, 0.08, -0.3, -0.26]
        region[:, 2] = [-1.93, -1.925, -0.08, 0.08, -0.2, -0.16]
        region[:, 3] = [-1.93, -1.925, -0.08, 0.08, -0.1, -0.06]
        region[:, 4] = [-1.93, -1.925, -0.08, 0.08, 0.0, 0.04]
        ...
        binRange = binRangeE
    elseif pType == 'i'
        region[:, 1] = [-1.93, -1.92, -0.08, 0.08, -0.3, -0.26]
        region[:, 2] = [-1.93, -1.92, -0.08, 0.08, -0.2, -0.16]
        region[:, 3] = [-1.93, -1.92, -0.08, 0.08, -0.1, -0.06]
        region[:, 4] = [-1.93, -1.92, -0.08, 0.08, 0.0, 0.04]
    ...
    for i in 1:(nBox * 2)
        ...
        if plottype[i] == 1
            if i in (4, 8)
                ax[i].annotate(
                    L"u_y", xy = (0.5, -0.2), xycoords = "axes fraction",
                    fontsize = fs
                )
            else
                ax[i].annotate(
                    L"u_y", xy = xlPos, xycoords = "axes fraction",
                    fontsize = fs
                )
            end
            ax[i].annotate(
                L"u_x", xy = ylPos, xycoords = "axes fraction",
                fontsize = fs
            )
        elseif plottype[i] == 2
            ax[i].annotate(
                L"u_x", xy = xlPos, xycoords = "axes fraction",
                fontsize = fs
            )
            ax[i].annotate(
                L"u_z", xy = ylPos, xycoords = "axes fraction",
                fontsize = fs
            )
        elseif plottype[i] == 3
            if i in (4, 8)
        ...
        elseif plottype[i] == 4
            ax[i].annotate(
                L"u_{\perp Out}", xy = xlPos, xycoords = "axes fraction",
                fontsize = fs
            )
            ax[i].annotate(
                L"u_{\perp In}", xy = ylPos, xycoords = "axes fraction",
                fontsize = fs
            )
        elseif plottype[i] == 5
            ax[i].annotate(
                L"u_{\perp In}", xy = xlPos, xycoords = "axes fraction",
                fontsize = fs
            )
            ax[i].annotate(
                L"u_\parallel", xy = ylPos, xycoords = "axes fraction",
                fontsize = fs
            )
        elseif plottype[i] == 6
            ax[i].annotate(
        ...
        if pType == 'e'
            ...
            ax[i].text(
                0.05, 0.05, string(iB), FontSize = fs, color = "r",
                transform = ax[i].transAxes
            )
        elseif pType == 'i'
            ax[i].text(
            ...
            ax[i].text(
                0.83, 0.05, string(iB + 4), FontSize = fs, color = "c",
                transform = ax[i].transAxes
            )
        end
    end
...
function show_box_region()
    ...
    region[:, 5] = [-1.93, -1.92, -0.08, 0.08, -0.3, -0.26]
    region[:, 6] = [-1.93, -1.92, -0.08, 0.08, -0.2, -0.16]
    region[:, 7] = [-1.93, -1.92, -0.08, 0.08, -0.1, -0.06]
    region[:, 8] = [-1.93, -1.92, -0.08, 0.08, 0.0, 0.04]

    c = ax.contourf(
        Z, X, Ex ./ 1.0e3, 100, norm = matplotlib.colors.DivergingNorm(0),
        vmin = -12.0e1, vmax = 12.0e1
    )
    ...
    for i in eachindex(xstart)
        ...
        xl[i],
            zl[i] = trace2d_rk4(
            Bx, Bz, xs, zs, x, z, ds = 0.02, maxstep = 20000,
            gridType = "ndgrid"
        )
    end
    ...
    for iB in 1:4
        rect = matplotlib.patches.Rectangle(
            (region[5, iB], region[1, iB]),
            region[6, iB] - region[5, iB], region[2, iB] - region[1, iB],
            lw = 1.0, fill = true, alpha = 0.9, fc = "r"
        ) # facecolor="none"
        ax.add_patch(rect)
    end
    ...
    for iB in 5:8
        rect = matplotlib.patches.Rectangle(
            (region[5, iB], region[1, iB]),
            region[6, iB] - region[5, iB], region[2, iB] - region[1, iB],
            lw = 1.0, fill = true, alpha = 0.9, fc = "c"
        ) # facecolor="none"
        ax.add_patch(rect)
    end

    ax.annotate("1", (region[5, 1] - 2.0e-2, region[2, 1]), fontsize = 10, color = "r")
    ax.annotate("2", (region[5, 2] - 2.0e-2, region[2, 2]), fontsize = 10, color = "r")
    ax.annotate("3", (region[5, 3] - 2.0e-2, region[2, 3] + 0.5e-2), fontsize = 10, color = "r")
    ax.annotate("4", (region[5, 4] - 2.0e-2, region[2, 4] + 1.0e-2), fontsize = 10, color = "r")

    for i in 5:8
        ax.annotate(
            string(i), (region[5, i] - 2.0e-2, region[2, i] + 0.2e-2), fontsize = 10,
            color = "c"
        )
    end
...
function HF_velocity()
    ...
    limits = [-1.91, -1.9, -0.08, 0.08, 0.11, 0.15]
