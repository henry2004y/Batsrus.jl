...
function extract_var(file::BatsrusHDF5Uniform{TI, TF}, var::String;
	...
	gslcx, xl_new, xu_new, ibx_ =
		prep_extract(file; dim = 1, vmin = xmin, vmax = xmax, step = stepx)
	gslcy, yl_new, yu_new, iby_ =
		prep_extract(file; dim = 2, vmin = ymin, vmax = ymax, step = stepy)
	gslcz, zl_new, zu_new, ibz_ =
		prep_extract(file; dim = 3, vmin = zmin, vmax = zmax, step = stepz)
