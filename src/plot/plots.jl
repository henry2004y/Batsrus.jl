...
@recipe function f(bd::BATS{1, TV, T}, var::AbstractString) where {TV, T}
   hasunits = hasunit(bd)
...
@recipe function f(bd::BATS{2, TV, T}, var::AbstractString;
   plotrange=[-Inf,Inf,-Inf,Inf], plotinterval=0.1) where {TV, T}
   hasunits = hasunit(bd)
