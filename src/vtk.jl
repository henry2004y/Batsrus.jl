...
function readhead(filehead)
    ...
    return Head(
        nI, nJ, nK, nG, iRatio, jRatio, kRatio, nDimAmr, nChild, nRoot_D,
    ...
    if !eof(io)
        ...
        if first_byte == UInt8('#') || (0x20 < first_byte < 0x7E)
            while !eof(io)
                ...
                parts = split(line)
                if length(parts) == 3 && all(p -> all(isdigit, p), parts)
                    seek(io, position(io) - length(line) - 2) # Rewind (considering \r\n)
                    break
                end
            end
        else
...
function order_children!(
        batl::Batl, iNode::Int32, iMorton::Int,
...
function getSibling(iNodeNei_III, iTree_IA)
    ...
    return iSibling = findfirst(x -> x == iMyNode, motherNode[child1_:end])
end
